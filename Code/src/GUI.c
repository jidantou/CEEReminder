...
const uchar Pause[6] = {0xFC, 0xFC, 0xFC, 0x3F, 0x3F, 0x3F};
...
{	
	OLED_Show(40, 2, 8, 2, Asc8x16[2], 0);
	OLED_Show(48, 2, 8, 2, Asc8x16[0], 0);	
	OLED_Show(56, 2, 8, 2, Asc8x16[Time[6] / 10], mode * ((CURSOR >> 0) & 0x01));	
	OLED_Show(64, 2, 8, 2, Asc8x16[Time[6] % 10], mode * ((CURSOR >> 0) & 0x01));
	OLED_Show(72, 2, 16, 2, Ch16x16[13], 0);

	OLED_Show(32, 4, 8, 2, Asc8x16[Time[4] / 10], mode * ((CURSOR >> 1) & 0x01));
	OLED_Show(40, 4, 8, 2, Asc8x16[Time[4] % 10], mode * ((CURSOR >> 1) & 0x01));
	OLED_Show(48, 4, 16, 2, Ch16x16[14], 0);
				
	OLED_Show(64, 4, 8, 2, Asc8x16[Time[3] / 10], mode * ((CURSOR >> 2) & 0x01));
	OLED_Show(72, 4, 8, 2, Asc8x16[Time[3] % 10], mode * ((CURSOR >> 2) & 0x01));
...
void GUI_Memu0()
{
	...
	OLED_Fill(104, 0, 127, 1, (CURSOR >> 0) & 0x01);
	OLED_Fill(72, 2, 127, 3, (CURSOR >> 1) & 0x01);
	OLED_Fill(72, 4, 127, 5, (CURSOR >> 2) & 0x01);
	OLED_Fill(96, 6, 127, 7, (CURSOR >> 3) & 0x01);
	/*
	switch(CURSOR)
	{
		case 1:
...
void GUI_Memu1()
{
	...
	OLED_Fill(96, 0, 127, 1, (CURSOR >> 0) & 0x01);
	OLED_Fill(40, 2, 127, 3, (CURSOR >> 1) & 0x01);
}

// DEBUG
/*
void GUI_Test(uint dat)
{
	OLED_Cls();
	OLED_Show(32, 2, 16, 4, Num16x32[dat / 100], 0);
	OLED_Show(48, 2, 16, 4, Num16x32[(dat % 100) / 10], 0);
	OLED_Show(64, 2, 16, 4, Num16x32[(dat % 100) % 10], 0);
}
*/